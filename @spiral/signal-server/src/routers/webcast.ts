...
import {SpiralData} from "../modules/ws/modules/schemas";
import {SPIRAL_ID_PATTERN} from "../modules/ws/manager";

export const webcastRoutes = Router();
...
webcastRoutes.post("/fetch", async (req: e.Request, res: e.Response): Promise<void> => {
  const roomId = req.query.room_id as string | null;
  const preferred_agent_id = req.query.preferred_agent_id as string | null;
  const agentManager = null as unknown as any;//  globalThis.WebSocketManager.agentManager; // todo use req.apps
