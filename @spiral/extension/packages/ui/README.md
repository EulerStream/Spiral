...
Add the following to the `tailwind.config.ts.test` file.
...
Modify the `tailwind.config.ts.test` file to make global style changes to the package.
...
```json
{
  ...
  "tsx": true,
  "tailwind": {
    "config": "tailwind.config.ts.test",
    "css": "lib/global.css",
    "baseColor": "neutral",
    "cssVariables": true,
    "prefix": ""
